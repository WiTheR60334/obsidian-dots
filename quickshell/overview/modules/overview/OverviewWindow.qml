...
Item { // Window
    ...
    readonly property bool workspaceHasNonFullscreenWindows: {
        const thisWsId = windowData?.workspace?.id;
        if (!Number.isFinite(thisWsId)) return false;
        return HyprlandData.windowList.some(w => w.workspace?.id === thisWsId && (w.fullscreen ?? 0) === 0);
    }
    readonly property real fullscreenSizeCap: (isFullscreen && workspaceHasNonFullscreenWindows) ? 0.75 : 1.0

    width: Math.min(targetWindowWidth, availableWorkspaceWidth) * fullscreenSizeCap
    height: Math.min(targetWindowHeight, availableWorkspaceHeight) * fullscreenSizeCap
